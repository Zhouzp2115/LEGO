...
public:
    Smodel(SmcContext *context, int batch_size = 128)
    {
        ...
        conv1 = new SConv2D(context, 3, 6, 5, batch_size, 32);
        conv1->genTriplets(3);
        pool1 = new SMaxPool2D(context, 2);
        conv2 = new SConv2D(context, 6, 16, 5, batch_size, 14);
        conv2->genTriplets(3);
        pool2 = new SMaxPool2D(context, 2);
        linear1 = new Slinear(context, 500, batch_size, 16 * 5 * 5, 128);
        linear1->init();
    ...
    void backward(int iter, Stensor sgrad)
    {
        ...
        sgrad.data = sgrad.data.reshape({batch_size, 16, 5, 5});
...
public:
    PlaintextMoel(SmcContext *context)
    {
        ...
        conv1 = new nn::Conv2d(nn::Conv2dOptions(3, 6, 5));
        conv2 = new nn::Conv2d(nn::Conv2dOptions(6, 16, 5));
        linear1 = new nn::LinearImpl(16 * 5 * 5, 128);
...
int main(int argc, char **argv)
{
    ...
    /***** load  data *****/
    int batchSize = 128;
    CifarDataLoader loader(context);

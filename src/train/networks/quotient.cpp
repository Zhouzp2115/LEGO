...
class PlaintextModel3
{
public:
    PlaintextModel3(SmcContext *context, int inputSize, int outputSize)
    {
        this->context = context;
        this->inputSize = inputSize;
        this->outputSize = outputSize;
        plinear1 = new PlaintextLinear(context, inputSize, 512);
        plinear2 = new PlaintextLinear(context, 512, 512);
        plinear3 = new PlaintextLinear(context, 512, outputSize);
...
{
public:
    SModel3(SmcContext *context, int batchSize)
    {
        this->context = context;
        linear1 = new Slinear(context, 6000, batchSize, 784, 512);
        linear2 = new Slinear(context, 6000, batchSize, 512, 512);
        linear3 = new Slinear(context, 6000, batchSize, 512, 10);
        linear1->init();
        linear2->init();
        linear3->init();
    ...
    Tensor test(PlaintextModel3 &plainModel, MnistDataLoader &loader)
    {
    ...
    void check(PlaintextModel3 &plainModel, MnistDataLoader &loader)
    {
        plainModel.plinear1->setWeight(linear1->getWeight().revealFloat());
        plainModel.plinear2->setWeight(linear2->getWeight().revealFloat());
        plainModel.plinear3->setWeight(linear3->getWeight().revealFloat());
...
int main(int argc, char **argv)
{
    ...
    /***** train on plaintext *****/
    PlaintextModel3 plainModel(context, 784, 10);
    //epoch=15 acc=0.97
    ...
    //securely train
    SModel3 smodel(context, batchSize);
    float all_time = 0;
